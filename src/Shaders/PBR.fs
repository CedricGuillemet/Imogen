...
uniform vec4 view;
uniform vec4 displacementFactor;
...
float DoorKnob( vec3 p, mat3 localToWorld )
{
    ...
    r = Substract( r, Box( t2, vec3(0.5, 0.5, 0.5) ) );
...
void main()
{
    ...
    vec3 col = textureCube(CubeSampler4, InvertCubeY(rd)).xyz;
	...
	*/
		...
		vec3 baseColor     = pow(boxmap(Sampler0, pos, normalize(pos), 1.0 ).xyz, vec3( 2.2, 2.2, 2.2) );
        ...
        vec3 env       = textureCube(CubeSampler4, InvertCubeY(refl), roughnessE*12.0).xyz;
    ...
    col = pow( col * .4, vec3( 1. / 2.2, 1./2.2, 1./2.2 ) );
