...
float DoorKnob( vec3 p, mat3 localToWorld )
{
    ...
    r = Substract( r, Box( t2, vec3( .5, .5, .5 ) ) );
...
void main()
{
    ...
    if ( t > 0.0 )
    {
		...
		vec3 albedo = texture2D(Sampler0, texcoord).xyz;
		...
		vec3 baseColor     = pow(albedo, vec3( 2.2, 2.2, 2.2 ) );

		float roughness = texture2D(Sampler3, texcoord).x;//boxmap(Sampler2, pos, normalize(pos), 1.0 ).x;
    ...
    #endif
    col = pow( col, vec3( 1. / 2.2, 1. / 2.2, 1. / 2.2 ) );
