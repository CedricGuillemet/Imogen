...
layout (std140) uniform PBR2Block
{
	vec2 view;
	float depthFactor;
//	int geometry;
...
float displace(vec3 p)
{
	...
	return -disp * PBR2Param.displacementFactor;
}
*/
...
vec4 PBR2()
{
	vec2 p = vUV * 2.0 - 1.0;
     ...
     // camera movement	
	float an = PBR2Param.view.x * PI * 2.0;
	float dn = PBR2Param.view.y * PI * 0.5;
	float cdn = cos(dn);
