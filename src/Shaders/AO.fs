...
uniform vec4 u_total_strength;// = 0.6;
uniform vec4 u_area;// = 0.0075;
uniform vec4 u_falloff;// = 0.00001;
uniform vec4 u_radius;// = 0.016;
...
void main()
{
	...
	// occ
	float radius_depth = u_radius.x/(depth+0.01);
	float occlusion = 0.0;
	for(int i=0; i < samples; i++) 
	{
		...
		occlusion += step(u_falloff.x, difference) * (1.0-smoothstep(u_falloff.x, u_area.x, difference));
	}
  
  
	float ao = 1.0 - u_total_strength.x * occlusion * (1.0 / float(samples));
